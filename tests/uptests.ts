...
async function requestContent(suffix: string = "", options: any = {}) {
    return await (
        await fetch(`http://localhost:4000${suffix}`, options)
    ).text();
}
...
registerTest("Middlewares working?", async () => {
    ...
    app.register((req, res) => {
        if (req.url === "/") {
            res.content =
                (res.content as string)?.replace("REPLACE_ME", "Planet") || "";
        }
        return res;
    });
    app.register((req, res) => {
...
registerTest("Regex routes working?", async () => {
    app.get(new RegExp("/hello-world/(.*)"), (req) =>
...
registerTest("Query schemas working?", async () => {
    ...
    const content = await requestContent(
        "/test-query-schema-working?hello=test"
    );
    if (content !== "Hello, World!")
...
registerTest(
    ...
    async () => {
        app.get(
            "/test-parameter-schema-failing/:hello",
            (req) => "Hello, World!",
            {
                schema: {
                    parameters: [
...
registerTest("File uploading working?", async () => {
    ...
    const content = await requestContent("/upload", {
        method: "post",
        body: f,
    });
    if (content !== "1255")
...
registerTest(
    ...
    async () => {
        ...
        if (
            valueMustBeOfType("test", "string").bind(schemaContext)(
                schemaContext
            )
        )
            throw Error("valueMustBeByType function returned wrong value");
